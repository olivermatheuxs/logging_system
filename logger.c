...
int main() {
  initialize_logging();
  HCT_FATAL("FATAL EXCEPTION OPENING MODULES %s and %s", "WINDOW", "RENDERER");
  HCT_ERROR("CATCH ERROR WITH %s", "VULKAN INSTANCE");
  HCT_WARN("THIS IS A WARN %s %s", "BE", "CAREFUL");
  HCT_INFO("THIS IS A INFO, NOT A POINTER %s", "(INT *)IAMAPOITER");
  HCT_DEBUG("DEBUG here %s", "well well");
  HCT_TRACE("OPENING WINDOW COMPATIBLE WITH %s", "WAYLAND");
}